...
# lower changes the number of bins in the faceted histograms in the bottom row (usually ignore)

# columns allows you to specify which columns you want to look at (very important)
...
# automatically obtains the classification rate from the confusion matrix:
...
# Exercise 15.3
```{r}
knn_mod2 <- knn(train = train_small, test = test_small, cl = train_cat, k = 9)
knn_mod2

table(knn_mod2, test_cat)
tab <- table(knn_mod, test_cat)
sum(diag(tab)) / sum(tab)
```
...
# Project 3
```{r}
# function that automates running knn with different k values
get_class_rate <- function(k = 10) {
  knn_modA <- knn(train = train_song_small, test = test_song_small,
                  cl = train_song, k = k)
  res_tab <- table(knn_modA, test_song)
  sum(diag(res_tab)) / sum(res_tab)
}

classification_rates <- map(1:50, get_class_rate) |> unlist()
class_rate_df <- tibble(ks = 1:50, classification_rates)
ggplot(data = class_rate_df, aes(x = ks, y = classification_rates)) + geom_line(size = 1)
class_rate_df |> arrange(desc(classification_rates)) |> slice(1)
```
